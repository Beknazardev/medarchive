"""Almaty CGKB adapter - extracts tariff tables from public HTML pages."""
...
def extract_almaty_cgkb(document: SourceDocument) -> ExtractionResult:
    """Extract tariff rows from an Almaty CGKB HTML price table page."""
    ...
    if not _validate_headings(header_row):
        errors.append(
            ExtractionError(
                ...
                code="SCHEMA_DRIFT",
                message="Table headings do not match expected CGKB schema",
                source_url=document.final_url,
            )
        )
        return ExtractionResult(
        ...
        candidates.append(
            RawServiceCandidate(
                ...
                raw_payload={
                    ...
                    "tariff_audience": "almaty_residents",
...
def transform_to_scraped_contract(
    ...
    *,
    ...
    robots_notes: str = "Public Almaty CGKB price page. F0 policy rechecked.",
) -> dict[str, Any]:
    ...
    for candidate in extraction.candidates:
        row = {
            "row_id": f"almaty_cgkb_row_{candidate.raw_payload.get('row_number', 0):03d}",
    ...
    return {
        ...
        "clinic": {
            ...
            "name": "ЦГКБ Алматы",
            "legal_name": "Центральная городская клиническая больница г. Алматы",
            "city": "Алматы",
            "address": None,
            "phone": None,
            "website": "https://almaty-cgkb.kz",
            "working_hours": None,
        },
        "branches": [],
        "rows": rows,
...
def _validate_headings(headings: list[str]) -> bool:
    """Validate that headings match expected CGKB schema."""
