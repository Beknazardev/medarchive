"""BMCUDP adapter - extracts tariff tables from public HTML pages."""
...
def extract_bmcudp(document: SourceDocument) -> ExtractionResult:
    """Extract tariff rows from a BMCUDP HTML price table page."""
    ...
    if not _validate_headings(header_row):
        errors.append(
            ExtractionError(
                ...
                code="SCHEMA_DRIFT",
                message="Table headings do not match expected BMCUDP schema",
                source_url=document.final_url,
            )
        )
        return ExtractionResult(
...
def transform_to_scraped_contract(
    ...
    *,
    ...
    robots_notes: str = "Public BMCUDP tariff page. F0 policy rechecked.",
) -> dict[str, Any]:
    ...
    for candidate in extraction.candidates:
        row = {
            "row_id": f"bmcudp_row_{candidate.raw_payload.get('row_number', 0):03d}",
    ...
    return {
        ...
        "clinic": {
            ...
            "name": "БМЦ УДП РК",
            "legal_name": "Больница Медицинского центра УДП РК",
            "city": "Астана",
            "address": None,
            "phone": None,
...
def _validate_headings(headings: list[str]) -> bool:
    """Validate that headings match expected BMCUDP schema."""
