"""KDL Olymp adapter - extracts price lists from public HTML pages."""
...
EXPECTED_HEADINGS = {"наименование", "название", "исследование", "услуга", "анализ"}
EXPECTED_PRICE_HEADINGS = {"цена", "стоимость", "тенге", "₽", "₸"}
MIN_ROWS = 3


def extract_kdl_olymp(document: SourceDocument) -> ExtractionResult:
    """Extract service/price rows from a KDL Olymp HTML price list page."""
    ...
    if not _validate_headings(header_row):
        errors.append(
            ExtractionError(
                ...
                code="SCHEMA_DRIFT",
                message="Table headings do not match expected KDL Olymp schema",
                source_url=document.final_url,
            )
        )
        return ExtractionResult(
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        duration_text = _get_cell_text(cells, col_map.get("duration"))
...
def transform_to_scraped_contract(
    ...
    *,
    ...
    robots_notes: str = "Public KDL Olymp price list page. F0 policy rechecked.",
) -> dict[str, Any]:
    ...
    for candidate in extraction.candidates:
        row = {
            "row_id": f"kdl_olymp_row_{candidate.raw_payload.get('row_number', 0):03d}",
...
def _validate_headings(headings: list[str]) -> bool:
    """Validate that headings match expected KDL Olymp schema."""
