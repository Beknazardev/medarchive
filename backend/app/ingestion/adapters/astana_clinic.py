"""Astana Clinic adapter - extracts tariff tables from public HTML pages."""
...
EXPECTED_UNIT_HEADINGS = {"ед", "единица", "измерения"}
EXPECTED_RK_PRICE_HEADINGS = {"цена рк", "цена рк (₸)", "цена рк", "тенге"}
MIN_ROWS = 3


def extract_astana_clinic(document: SourceDocument) -> ExtractionResult:
    """Extract RK tariff rows from an Astana Clinic HTML price table page."""
    ...
    if not _validate_headings(header_row):
        errors.append(
            ExtractionError(
                ...
                code="SCHEMA_DRIFT",
                message="Table headings do not match expected Astana Clinic schema",
                source_url=document.final_url,
            )
        )
        return ExtractionResult(
    ...
    if "rk_price" not in col_map:
        errors.append(
            ExtractionError(
                source_id=SOURCE_ID,
                stage=ParserStage.VALIDATE,
                code="NO_RK_TARIFF_COLUMN",
                message="RK tariff price column not found in table",
                source_url=document.final_url,
            )
        )
        return ExtractionResult(
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        unit = _get_cell_text(cells, col_map.get("unit"))
        price_text = _get_cell_text(cells, col_map.get("rk_price"))
...
def transform_to_scraped_contract(
    ...
    *,
    ...
    robots_notes: str = "Public Astana Clinic price page. F0 policy rechecked.",
) -> dict[str, Any]:
    ...
    for candidate in extraction.candidates:
        row = {
            "row_id": f"astana_clinic_row_{candidate.raw_payload.get('row_number', 0):03d}",
            ...
            "unit": candidate.raw_payload.get("unit"),
            "section": candidate.raw_payload.get("section"),
            "tariff_audience": candidate.raw_payload.get("tariff_audience"),
            "is_available": True,
            "raw": {
    ...
    return {
        ...
        "clinic": {
            ...
            "address": None,
            "phone": None,
            "website": "https://www.astanaclinic.kz",
            "working_hours": None,
        },
        "branches": [],
        "rows": rows,
...
def _validate_headings(headings: list[str]) -> bool:
    """Validate that headings match expected Astana Clinic schema."""
    lower_headings = [h.lower() for h in headings]
    has_name = any(
        any(kw in h for kw in EXPECTED_HEADINGS) for h in lower_headings
    )
    has_price = any(
        any(kw in h for kw in EXPECTED_RK_PRICE_HEADINGS) for h in lower_headings
    )
    return has_name and has_price


def _map_columns(headings: list[str]) -> dict[str, int]:
    """Map column roles to indices. RK price column is selected."""
    ...
    for idx, h in enumerate(headings):
        ...
        if any(kw in hl for kw in {"№", "п/п", "номер", "порядковый"}):
            ...
            mapping["name"] = idx
        elif any(kw in hl for kw in EXPECTED_UNIT_HEADINGS):
            mapping["unit"] = idx
        elif any(kw in hl for kw in EXPECTED_RK_PRICE_HEADINGS):
            if "rk_price" not in mapping:
                mapping["rk_price"] = idx
    return mapping
